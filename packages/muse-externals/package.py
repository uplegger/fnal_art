...
class MuseExternals(BundlePackage):
    ...
    def setup_run_environment(self, env):
        ...
        env.set("ART_ROOT_IO_INC", self.spec["art_root_io"].prefix.include)
        ...
        env.set("CETLIB_EXCEPT_INC", self.spec["cetlib_except"].prefix.include)
        env.set("CETLIB_INC", self.spec["cetlib"].prefix.include)
        env.set("CET_PLUGIN_PATH", self.spec["cetlib_except"].prefix.include)
        ...
        env.set("HEP_CONCURRENCY_INC", self.spec["hep_concurrency"].prefix.include)
        env.set("KINKAL_INC", self.spec["KinKal"].prefix.include)
        env.set("MESSAGEFACILITY_INC", self.spec["messagefacility"].prefix.include)
        env.set("MU2E_PCIE_UTILS_INC", self.spec["mu2e_pcie_utils"].prefix.include)
        ...
        env.set("TBB_INC", self.spec["tbb"].prefix.include)
        ...
        env.set("XERCES_C_INC", self.spec["xerces_c"].prefix.include)
